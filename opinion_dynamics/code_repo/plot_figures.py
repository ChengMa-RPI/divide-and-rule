...
def pA_pcA_p_heatmap(number_opinion, p_B_plot, plot_ABC):
    """plot P_A with P_{cA} for different p_{cB}
    ...
    for p in p_B_plot:
        ...
        if plot_ABC == 'A':
            ...
            y = x_B_p[index_sort] + p_cB_p[index_sort]
            ylabel = '$n_C$'
        x_plot_list.append(y)
    pcA_sort = p_cA_p[index_sort]
    x_plot_list = np.array(x_plot_list).transpose()[::-1]
    ax = sns.heatmap(x_plot_list, cbar_kws={'label': ylabel} )
    ax.figure.axes[-1].yaxis.label.set_size(22)
    ax.figure.axes[-1].tick_params(labelsize=16)


    ax.set_yticks(np.arange(len(pcA_sort)) - 0.5)
    ax.set_yticklabels(np.round(pcA_sort[::-1], 2), rotation=-0)
    ax.set_xticks(np.arange(len(p_B_plot)) )
    ax.set_xticklabels(p_B_plot, rotation=0)


    plt.xlabel('$P_C$', fontsize=fontsize)
    plt.ylabel('$P_A$', fontsize=fontsize)
    ...
    #legend = plt.legend(frameon=False, fontsize = legendsize, loc='upper right', markerscale=1.5, bbox_to_anchor=(1.45, 1.))
    plt.locator_params(nbins=5)
    save_des = f'../figure0213/n{plot_ABC}_PA_N=3_heatmap.png'
    plt.savefig(save_des)
    plt.close()
    #plt.show()
...
def pA_pcA_p_approximation_heatmap(number_opinion, p_not_A, plot_ABC):
    """plot P_A with P_{cA} for different p_{cB}
    ...
    ax.set_yticks(np.arange(len(pcA_sort)) - 0.5)
    ax.set_yticklabels(np.round(pcA_sort[::-1], 2), rotation=-0)
    ax.set_xticks(np.arange(len(p_not_A)) )
    ax.set_xticklabels(p_not_A, rotation=0)
    ...
    plt.subplots_adjust(left=0.18, right=0.91, wspace=0.25, hspace=0.25, bottom=0.15, top=0.95)
    plt.legend(frameon=False, fontsize = legendsize, markerscale=1.0, loc='lower right', bbox_to_anchor=(1.48, -0.10) )
    #plt.legend(frameon=False, fontsize = legendsize, markerscale=3.0)
    plt.locator_params(nbins=5)
    save_des = f'../figure0213/n{plot_ABC}_PA_N={number_opinion}_heatmap.png'
    plt.savefig(save_des)
    plt.close()
    #plt.show()
...
"Figure 2 -- revised, change plot_ABC to show the fraction of any one the opinions v.s. p_A"
number_opinion = 3
p_B_plot = np.round(np.arange(0, 0.3, 0.03), 4 )
plot_ABC = 'B'
pA_pcA_p_heatmap(number_opinion, p_B_plot, plot_ABC)
...
"figure 3 - revised for heatmap"
number_opinion = 6
for plot_ABC in ['A', 'B', 'C']:
    #pA_pcA_p_approximation_heatmap(number_opinion, p_not_A, plot_ABC)
    pass
